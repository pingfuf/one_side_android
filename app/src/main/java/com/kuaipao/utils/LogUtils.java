...
import static com.kuaipao.utils.LangUtils.*;
...
public class LogUtils {
    public static final String LOG_ID = "Card";

    public static final boolean V = Log.VERBOSE >= CardConfig.MIN_LOG_LEVEL;
    public static final boolean D = Log.DEBUG >= CardConfig.MIN_LOG_LEVEL;
    public static final boolean I = Log.INFO >= CardConfig.MIN_LOG_LEVEL;
    public static final boolean W = Log.WARN >= CardConfig.MIN_LOG_LEVEL;
    public static final boolean DW = CardConfig.DEV_BUILD && Log.WARN >= CardConfig.MIN_LOG_LEVEL;
    public static final boolean E = Log.ERROR >= CardConfig.MIN_LOG_LEVEL;

    private static void log(int priority, String msg, Throwable throwable) {
        try {
            if (throwable != null) {
                Log.println(priority, LOG_ID,
                        releaseStringBuilder(acquireStringBuilder(0)
                                .append(msg)
                                .append(", exception:\n\t")
                                .append(
                                        (priority >= Log.ERROR || CardConfig.DEV_BUILD) ? Log.getStackTraceString(throwable)
    ...
    public static void v(Throwable throwable, String format, Object... args) {
        if (Log.VERBOSE >= CardConfig.MIN_LOG_LEVEL)
            try {
     ...
     * @param args
     */
    public static void d(Throwable throwable, String format, @CheckForNull Object... args) {
        if (Log.DEBUG >= CardConfig.MIN_LOG_LEVEL) {
            try {
     ...
     * @param args
     */
    public static void i(Throwable throwable, String format, @CheckForNull Object... args) {
        if (Log.INFO >= CardConfig.MIN_LOG_LEVEL) {
            try {
                if (args == null || args.length == 0)
     ...
     * @param args
     */
    public static void w(Throwable throwable, String format, @CheckForNull Object... args) {
        if (Log.WARN >= CardConfig.MIN_LOG_LEVEL) {
            try {
                if (args == null || args.length == 0)
     ...
     * @param args
     */
    public static void e(@CheckForNull Throwable throwable, String format, @CheckForNull Object... args) {
        if (Log.ERROR >= CardConfig.MIN_LOG_LEVEL) {
            try {
                String msg = format;
                if (args != null && args.length > 0)
     ...
     * @param args
     */
    public static void dw(Throwable throwable, String format, @CheckForNull Object... args) {
        if (DW)
            try {
                String msg = format;
                if (args != null && args.length > 0)
