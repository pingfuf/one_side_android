...
public class StringRequestPriority extends StringRequest {
     ...
     */
    public StringRequestPriority(int method, String url, Listener<String> listener,
     ...
     * @param errorListener Error listener, or null to ignore errors
     */
    public StringRequestPriority(String url, Listener<String> listener, ErrorListener errorListener) {
    ...
    protected Response<String> parseNetworkResponse(NetworkResponse response) {
        ...
        if (getUrl().contains("login") && response.statusCode == 200) {
            //登陆结果返回的时候，检测本地token
            String content = LangUtils.utf8String(response.data, "");
            if (LangUtils.isNotEmpty(content)) {
                JSONObject j = WebUtils.parseJsonObject(content);
                int code = WebUtils.getJsonInt(j, UrlRequest.CODE, 0);
                if (code == 0) {
