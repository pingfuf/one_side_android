 ...
 */
public class StoryPicAdapter extends XSimpleAdapter<ArticleSummary> {
    public StoryPicAdapter(Context context, List<ArticleSummary> data) {
        super(context, data);
    }
    ...
    private void setData(ViewHolder holder, ArticleSummary item) {
        ...
        if(!LangUtils.isEmpty(item.thumbnailList)) {
            holder.ivHeader.setImageUri(item.thumbnailList.get(0));
        }
