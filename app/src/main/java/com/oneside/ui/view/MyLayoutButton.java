...
public class MyLayoutButton extends View {
    ...
    public MyLayoutButton(Context context) {
        this(context, null);
    }

    public MyLayoutButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }


    @SuppressLint("NewApi")// api >= 11
    public MyLayoutButton(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mPadding = /*getResources().getDimensionPixelSize(R.dimen.paper_padding);*/0;
        ...
        mTextSize = getResources().getDimensionPixelSize(R.dimen.paper_text_size);/*attributes.getDimensionPixelSize(R.styleable.PaperButton_paper_text_size,
            getResources().getDimensionPixelSize(R.dimen.paper_text_size));*/
        mTextColor = getResources().getColor(R.color.orange);/*attributes.getColor(R.styleable.PaperButton_paper_text_color,
            getResources().getColor(R.color.paper_text_color));*/
        mTextColor2 = getResources().getColor(R.color.gray);
        ...
        textPaint2.setColor(mTextColor2);
        textPaint2.setTextSize(mTextSize);
        textPaint2.setTextAlign(TextPaint.Align.CENTER);
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                mMoveOutside = false;
                int which = (int) (event.getX() * 2 / getWidth());
                mFingerRect = new Rect(getLeft(), getTop(), getRight(), getBottom());
//                mTouchPoint.set(Math.round(event.getX()), Math.round(event.getY()));
                mTouchPoint.set(Math.round(/*event.getX()*/((which * 2 + 1) * getWidth()) / (2 * 2)
                ), Math.round(/*event.getY()*/getHeight() / 2));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        switch (mState) {
            ...
            case StateTouchDown:
                ...
                if (elapsed < ANIMATION_DURATION) {
                    radius = Math.round(elapsed * getWidth() / 6 / ANIMATION_DURATION);
            ...
            case StateTouchUp:
                if (elapsed < ANIMATION_DURATION) {
                    ...
                    radius = getWidth() / 6 + Math.round(elapsed * getWidth() / 6 / ANIMATION_DURATION);
                    float shadowAlpha = (MAX_SHADOW_COLOR_ALPHA - MIN_SHADOW_COLOR_ALPHA)
        ...
        if (mText != null && mText.length() > 0) {
            ...
            int x = (int) (getWidth() / 4 /*+ textPaint.measureText(getResources().getString(R.string.ready_to_start))*/);
            canvas.drawText(mText.toString(), x, y, textPaint);
        }
        if (mText2 != null && mText2.length() > 0) {
            int y = (int) (getHeight() / 2 - ((textPaint2.descent() + textPaint2.ascent()) / 2));
            int x = (int) (getWidth() * 3 / 4 /*+ textPaint.measureText(getResources().getString(R.string.finished))*/);
            canvas.drawText(mText2.toString(), x, y, textPaint2);
        }
    }
}
