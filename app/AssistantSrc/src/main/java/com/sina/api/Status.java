...
//    public Geo geo;
    ...
    public static Status parse(JSONObject jsonObject) {
        ...
        status.id = WebUtils.getJsonString(jsonObject, "id");
        ...
        status.favorited = WebUtils.getJsonBoolean(jsonObject, "favorited", false);
        status.truncated = WebUtils.getJsonBoolean(jsonObject, "truncated", false);
        ...
        status.retweeted_status = Status.parse(WebUtils.getJsonString(jsonObject, "retweeted_status"));
        ...
        status.mlevel = WebUtils.getJsonInt(jsonObject, "mlevel", -1);    // Have NOT supported
        status.visible = Visible.parse(WebUtils.getJsonObject(jsonObject, "visible"));

        JSONArray picUrlsArray = WebUtils.getJsonArray(jsonObject, "pic_urls");
        if (picUrlsArray != null && picUrlsArray.size() > 0) {
            int length = picUrlsArray.size();
            status.pic_urls = new ArrayList<String>(length);
            JSONObject tmpObject = null;
            for (int ix = 0; ix < length; ix++) {
                tmpObject = picUrlsArray.getJSONObject(ix);
                if (tmpObject != null) {
                    status.pic_urls.add(WebUtils.getJsonString(tmpObject, "thumbnail_pic"));
                }
            }
        }
